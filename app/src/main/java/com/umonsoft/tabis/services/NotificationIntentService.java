...
public class NotificationIntentService extends IntentService {
  ...
  protected void onHandleIntent(@Nullable final Intent intent) {
	 ...
	 editorNotification = preferencesNotification.edit();
	 ...
	 phpValues.get1Parameter(NotificationIntentService.this, sqlcodeAddingType, response -> {
  ...
  private void getRecordHistory(final VolleyGetRecordHistory callback) {
	 StringRequest stringRequest = new StringRequest(Request.Method.DEPRECATED_GET_OR_POST, "http://" + Constants.BASE_URL + "/getrecordhistory.php", response -> {
  ...
  private void sendPhpNotify(String sqlcode, final String prefInside, final String pref2Inside, final int addingtype) {
	 phpValues.get2Parameter(NotificationIntentService.this, sqlcode, (parameter1, parameter2) -> {
		...
		//eğer kişinin departmanına yeni kayıt eklenirse ve o kayıt önceki maxid'den büyükse
		{
		  ...
		  getRecordHistory((user_name, record_id, prevdepart, nextdepart, description, addingType) -> {
